...
Один workflow, по образцу Rabbit P2P (`/home/user/rebit-p2p/.github/workflows/makefile.yml`). **Ветка деплоя — `main` (§14.10): текущую `master` переименовать в `main` до создания workflow** (GitHub-side шаг, §13) — иначе триггеры/if-гейты молча не сработают ни разу. **Триггеры:** `push`/`pull_request` в `main`. На PR — только проверки; **build/push/deploy — только `push` в `main`** (`if: github.event_name == 'push' && github.ref == 'refs/heads/main'`). Джобы опираются на цели `Makefile`.
...
Выполнено ранее: ~~реструктуризация в монорепо + git-репозиторий~~ (сделано; ветка пока `master` — переименование в шаге 4), ~~`symfony/console`~~ (сделано — `bin/app.php`), `pdo_pgsql` в dev-образе есть.
...
4. **GitHub-side setup:** переименование ветки `master → main` + default branch; branch protection; environment `production` (+ required reviewers); secrets/vars по списку §6; `permissions: packages: write`.
...
8. **Бэкап-пререквизиты** (§10: бакет YOS + lifecycle + IAM-ключи; образ backup) + минимальные алерты + ротация docker-логов (`logging:` в compose) + Sentry (PII-scrubbing, §16.4) — **до** первого прод-деплоя.
...
10. **Ветка деплоя — `main`:** текущую `master` переименовать (`git branch -m` + default branch на GitHub) на шаге GitHub-side setup (§13.4) — до создания workflow. Совпадает с эталоном P2P и всеми `if`-гейтами §6.
